<script>
  ...
  import { selectedBar, 
    ...
    apiAlignments,
    originalTabSizes,
    legendInfo } from '../store/store'
  import { getDistanceMatrix, 
    ...
    getAlignmetnNotes,
    areArraysEqual,
    getTechniquesColors } from '../utils/lib';
  ...
  //value for first time rendering
  let firstTime = false;
  ...
  $: {
    if (($selectedCriteria !== $previousCriteria) && $selectedCriteria != '') {
      ...
      if (noteCollections.length !== 0) {
          ...
          Promise.all(
            apis.map((element, i) => {
              ...
              return waitForSvg(svg, main).then(svg => {
                ...
                return {
                  id: i,
                  y: api.renderer.boundsLookup._masterBarLookup.entries().next().value[1].lineAlignedBounds.y,
            ...
            .then(info => {
              ...
              const filteredInfo = tempInfo.filter((item) => $tabOrder.includes(item.id));
        ...
        //     const colorScale = colors.splice(0, api.renderer.boundsLookup._masterBarLookup.size);
        //     colorsForZoom.push(colorScale);
        //     colorizeBars(api, svg, i, colorScale);
        //     $overviewInfo = [
        //       ...$overviewInfo,
        //       {
        //         id: i,
  ...
  const colorizeNotes = (api, markedNotes, id) => {
    ...
    const bars = d3.select(api.canvasElement.element)
                    .append('svg')
    ...
    //       bars
    //       .append("rect")
    //       .attr("class", `coloredMeasure measure${markedNotes[realNoteCount].barNumber} isChord${markedNotes[realNoteCount].isChord} ${markedNotes[realNoteCount].color}`)
    //       .attr("x", markedNotes[realNoteCount].x - 3)
    //       .attr("y", markedNotes[realNoteCount].y - 18)
    //       .attr("width", markedNotes[realNoteCount].w + 5)
    //       .attr("height", markedNotes[realNoteCount].h - 5)
    //       .attr("rx", markedNotes[realNoteCount].isChord === '0' ? 20 : 0)
    //       .attr("fill", markedNotes[realNoteCount].color)
    //       .attr("stroke", markedNotes[realNoteCount].color)
    //       .attr("fill-opacity", 0.1)
    //       realNoteCount ++;
    //   }
    // }
    markedNotes.map((note, i) => {
      bars
        .append("rect")
        .attr("class", `coloredMeasure measure${note.barNumber} isChord${note.isChord} ${note.color}`)
        .attr("x", note.x - 3)
        .attr("y", note.y - 18)
        .attr("width", note.w + 5)
  ...
  const colorizeBars = (api, svg, id, colors) => {
    ...
    const masterBars = api.renderer.boundsLookup._masterBarLookup;
    // console.log(masterBars, 'tamano de barrraaaas')
    ...
    for (let [key, value] of masterBars) {
      if(sequence.alignment[key] !== '-') {
        ...
        bars
          ...
          .attr("fill", $selectedCriteria !== 'techniques' ? colors[realColorCount]:'white')
          .attr("stroke", "black")
          .attr("stroke-width", 1)
  ...
  const getNotes = (bars) => {
    ...
    // bars.map((bar, i) => {
    //   bar.voices[0].beats.map((beat, beatNumber) => {
  ...
  const colorizeNotesforBarUpdate = () => {
    let notesForOne = [];
    apis.forEach((api) => {
        const notes = getNotes(api.content.renderer.boundsLookup._masterBarLookup);
  ...
  const renderTabs = async (routes) => {
    ...
    // };
    // customApi = new window.alphaTab.AlphaTabApi(customMain, apiSettings2);
    routes.forEach((route, i) => {
      const apiSettings = {
        ...settings, // copy the settings object
        // file: route, // set the file property to the current route
      };
      api = new window.alphaTab.AlphaTabApi(main, apiSettings);
      // console.log(customApi, 'custooom')
      api.load(route.buffer);
      // try {
      //   api.beatMouseUp.on((beat) => {
      //     console.log(customApi)
      //     let clickedBar = beat.voice.bar
      //     let currentBar = customApi.tracks[0].staves[0].bars[customApi.tracks[0].staves[0].bars.length - 1];
      //     let voiceDiff = currentBar.voices.length - clickedBar.voices.length;
      //     if(voiceDiff > 0) {
      //       console.log('holaaaa')
      //       for(let j = 0; j < voiceDiff; j ++) {
      //         clickedBar.addVoice(new alphaTab.model.Voice());
      //         clickedBar.voices[j + 1].addBeat(new alphaTab.model.Beat());
      //       }
      //     }
      //     console.log(clickedBar.voices, 'clickeeeed');
      //     console.log(currentBar.voices, 'custooooooooooom')
      //     currentBar.staff.track.score.addMasterBar(new alphaTab.model.MasterBar());
      //     currentBar.staff.addBar(clickedBar);
      //     customApi.render()
      //   });
      // } catch (error) {
      //   console.log('alphatab la regooo')
      // }
      $selectedTracks.forEach((element) => {
        console.log(element, i)
        if(i === element.api) {
          api.renderTracks([api.score.tracks.find(e => e.name === element.name)])
        }
      })
      apis.push({
        id: route.id,
  ...
  afterUpdate(async () => {
    if (($tabRoutes !== previousRoutes) || ($selectedTracks !== previousSelectedTracks)) {
      colors = [];
      colorsForZoom = [];
      zoom = defaultZoom;
    ...
    apis.map((api)=> {
      ...
      for(let i = 0; i < sequence.alignment.length; i++) {
        if(sequence.alignment[i] === '-') {
          ...
          referenceScore = addMasterBar(new alphaTab.model.MasterBar(), temporalScore, i);
...
<div class="main-container">
  <div class="customize-container">
    <div class="bar-size-container">
      <button class="ui icon button button-icon" on:click={() => {handleBarWidth('-')}} disabled = {barWidth === 100}>
        <i class="minus icon"></i>
      </button>
        Bar Width: {`${barWidth}px `}
      <button class="ui icon button button-icon" on:click={() => {handleBarWidth('+')}}>
        <i class="plus icon"></i>
      </button>
    </div>
    <div class="zoom-container">
      <button class="ui icon button button-icon" on:click={() => {handleZoom('-')}}>
  ...
  <!-- <div class="legend-container">
    {#if colors && ($selectedCriteria !== "similarity" && $selectedCriteria !== "1 on 1 comparison" && $selectedCriteria !== "techniques") && $selectedCriteria !== ''}
      <ColorLegend color={d3.scaleSequential([minForLegend, maxForLegend], colorScaleForLegend)} title={$selectedCriteria}/>
    {/if}
  </div> -->
  <div>
    {#if $selectedCriteria === "1 on 1 comparison"}
  ...
  <div class="content-container">
    <div class="tab-container" bind:this="{wrapper}" on:scroll={setBarIndicator}>
