<script>
    ...
    import TechniquesLegend from './TechniquesLegend.svelte';
    import { overviewInfo, tabRoutes, selectedCriteria, legendInfo } from '../store/store';
        ...
        <div class="comparison-container" slot="left">
            ...
            <div class="legend-container">
                {#if ($selectedCriteria !== "similarity" && $selectedCriteria !== "1 on 1 comparison" && $selectedCriteria !== "techniques") && $selectedCriteria !== ''}
                    <ColorLegend color={d3.scaleSequential([$legendInfo.minForLegend, $legendInfo.maxForLegend], $legendInfo.colorScaleForLegend)} title={$selectedCriteria}/>
                {/if}
            </div>
            <div class="comparison-block">
                <h2 class="overview-header">
        ...
        <div class="track-container" slot="right">
            ...
            {#if $tabRoutes.length === 0}
                <div class="ui placeholder segment tab-placeholder">
                    <div class="ui icon header">
                        Upload your tabs and start comparing
                    </div>
                </div>
            {:else}
                <TabDisplay/>
